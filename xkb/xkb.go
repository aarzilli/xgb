...
func ActionUnionRead(buf []byte, v *ActionUnion) int {
	...
	{
		var x SASetMods
		SASetModsRead(buf[b:], &x)
		v.Latchmods = SALatchMods(x)
	}
	...
	{
		var x SASetGroup
		SASetGroupRead(buf[b:], &x)
		v.Latchgroup = SALatchGroup(x)
	}
	...
	{
		var x SASetControls
		SASetControlsRead(buf[b:], &x)
		v.Lockcontrols = SALockControls(x)
	}
...
func BehaviorUnionRead(buf []byte, v *BehaviorUnion) int {
	...
	b = 0 // re-read the same bytes
	{
		var x DefaultBehavior
		DefaultBehaviorRead(buf[b:], &x)
		v.Lock = LockBehavior(x)
	}
	...
	b = 0 // re-read the same bytes
	{
		var x DefaultBehavior
		DefaultBehaviorRead(buf[b:], &x)
		v.PermamentLock = PermamentLockBehavior(x)
	}

	b = 0 // re-read the same bytes
	{
		var x RadioGroupBehavior
		RadioGroupBehaviorRead(buf[b:], &x)
		v.PermamentRadioGroup = PermamentRadioGroupBehavior(x)
	}

	b = 0 // re-read the same bytes
	{
		var x OverlayBehavior
		OverlayBehaviorRead(buf[b:], &x)
		v.PermamentOverlay1 = PermamentOverlayBehavior(x)
	}
	...
	b = 0 // re-read the same bytes
	{
		var x OverlayBehavior
		OverlayBehaviorRead(buf[b:], &x)
		v.PermamentOverlay2 = PermamentOverlayBehavior(x)
	}
...
type DeviceLedInfo struct {
	...
	State          uint32
	Names          []xproto.Atom  // size: xgb.Pad((xgb.PopCount(int(NamesPresent)) * 4))
	Maps           []IndicatorMap // size: xgb.Pad((xgb.PopCount(int(MapsPresent)) * 12))
}
...
func DeviceLedInfoRead(buf []byte, v *DeviceLedInfo) int {
	...
	v.Names = make([]xproto.Atom, xgb.PopCount(int(v.NamesPresent)))
	for i := 0; i < int(xgb.PopCount(int(v.NamesPresent))); i++ {
		v.Names[i] = xproto.Atom(xgb.Get32(buf[b:]))
		b += 4
	}

	v.Maps = make([]IndicatorMap, xgb.PopCount(int(v.MapsPresent)))
...
func (v DeviceLedInfo) Bytes() []byte {
	buf := make([]byte, ((20 + xgb.Pad((xgb.PopCount(int(v.NamesPresent)) * 4))) + xgb.Pad((xgb.PopCount(int(v.MapsPresent)) * 12))))
	b := 0
...
func DeviceLedInfoListSize(list []DeviceLedInfo) int {
	size := 0
	for _, item := range list {
		size += ((20 + xgb.Pad((xgb.PopCount(int(item.NamesPresent)) * 4))) + xgb.Pad((xgb.PopCount(int(item.MapsPresent)) * 12)))
	}
	return size
}
...
func KeyTypeRead(buf []byte, v *KeyType) int {
	...
	v.Preserve = make([]ModDef, (xgb.Bool2int(v.HasPreserve) * int(v.NMapEntries)))
...
func (v KeyType) Bytes() []byte {
	buf := make([]byte, ((8 + xgb.Pad((int(v.NMapEntries) * 8))) + xgb.Pad(((xgb.Bool2int(v.HasPreserve) * int(v.NMapEntries)) * 4))))
	b := 0
...
func KeyTypeListSize(list []KeyType) int {
	size := 0
	for _, item := range list {
		size += ((8 + xgb.Pad((int(item.NMapEntries) * 8))) + xgb.Pad(((xgb.Bool2int(item.HasPreserve) * int(item.NMapEntries)) * 4)))
	}
	return size
}
...
func SetKeyTypeRead(buf []byte, v *SetKeyType) int {
	...
	v.PreserveEntries = make([]KTSetMapEntry, (xgb.Bool2int(v.Preserve) * int(v.NMapEntries)))
...
func (v SetKeyType) Bytes() []byte {
	buf := make([]byte, ((8 + xgb.Pad((int(v.NMapEntries) * 4))) + xgb.Pad(((xgb.Bool2int(v.Preserve) * int(v.NMapEntries)) * 4))))
	b := 0
...
func SetKeyTypeListSize(list []SetKeyType) int {
	size := 0
	for _, item := range list {
		size += ((8 + xgb.Pad((int(item.NMapEntries) * 4))) + xgb.Pad(((xgb.Bool2int(item.Preserve) * int(item.NMapEntries)) * 4)))
	}
	return size
}
...
type GetCompatMapReply struct {
	...
	GroupRtrn []ModDef       // size: xgb.Pad((xgb.PopCount(int(GroupsRtrn)) * 4))
}
...
func getCompatMapReply(buf []byte) *GetCompatMapReply {
	...
	v.GroupRtrn = make([]ModDef, xgb.PopCount(int(v.GroupsRtrn)))
...
type GetIndicatorMapReply struct {
	...
	NIndicators    byte
	// padding: 15 bytes
	Maps []IndicatorMap // size: xgb.Pad((xgb.PopCount(int(Which)) * 12))
}
...
func getIndicatorMapReply(buf []byte) *GetIndicatorMapReply {
	...
	v.Maps = make([]IndicatorMap, xgb.PopCount(int(v.Which)))
	b += IndicatorMapReadList(buf[b:], v.Maps)
...
func selectEventsRequest(c *xgb.Conn, DeviceSpec DeviceSpec, AffectWhich uint16, Clear uint16, SelectAll uint16, AffectMap uint16, Map uint16, Details []uint32) []byte {
	size := xgb.Pad((16 + xgb.Pad((4 * xgb.PopCount(int((int(AffectWhich) & ((^(int(Clear))) & (^(int(SelectAll)))))))))))
	b := 0
	buf := make([]byte, size)
	...
	for i := 0; i < xgb.PopCount(int((int(AffectWhich) & ((^(int(Clear))) & (^(int(SelectAll))))))); i++ {
...
func setCompatMapRequest(c *xgb.Conn, DeviceSpec DeviceSpec, RecomputeActions bool, TruncateSI bool, Groups byte, FirstSI uint16, NSI uint16, Si []SymInterpret, GroupMaps []ModDef) []byte {
	size := xgb.Pad(((16 + xgb.Pad((int(NSI) * 17))) + xgb.Pad((xgb.PopCount(int(Groups)) * 4))))
	b := 0
	buf := make([]byte, size)
...
func setIndicatorMapRequest(c *xgb.Conn, DeviceSpec DeviceSpec, Which uint32, Maps []IndicatorMap) []byte {
	size := xgb.Pad((12 + xgb.Pad((xgb.PopCount(int(Which)) * 12))))
	b := 0
	buf := make([]byte, size)
